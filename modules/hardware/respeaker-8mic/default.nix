{
  ...
  cfg = config.raspberry-pi.hardware.respeaker-8mic;
  seeed-voicecard = pkgs.callPackage ../../../packages/seeed-voicecard {kernel = config.boot.kernelPackages.kernel;};
in {
  options.raspberry-pi.hardware.respeaker-8mic = {
    enable = lib.mkEnableOption ''
      support for the Seeed Studio ReSpeaker 8-Mic Array for Raspberry Pi
    '';
  };
  config = lib.mkIf cfg.enable {
    ...
    hardware.deviceTree = {
      ...
      overlays = [
        {
          name = "respeaker-8mic";
          dtsFile = "${seeed-voicecard}/lib/dts/seeed-8mic-voicecard-overlay.dts";
        }
        {
          name = "spi";
          dtsText = ''
